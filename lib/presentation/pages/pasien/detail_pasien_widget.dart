...
class DetailPasienWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    context.read<PasienBloc>().add(PasienEvent.getDetailPasien(noRM: norm));
        ...
        builder: (context, state) {
          ...
          return state.detailPasienResult.fold(
              ...
              (a) => a.fold(
                    (l) => l.maybeMap(
                      orElse: () => Container(),
                      badResponse: (value) => const DisconectWidget(
                          networkResponse: NetworkResponse.badRequest),
                      connectionTimeOut: (value) => const DisconectWidget(
                          networkResponse: NetworkResponse.timeOut),
                      disconectToServer: (value) => const DisconectWidget(
                          networkResponse: NetworkResponse.noConnection),
                      failure: (value) => const DisconectWidget(
                          networkResponse: NetworkResponse.failed),
                      noConnection: (value) => const DisconectWidget(
                          networkResponse: NetworkResponse.noConnection),
                    ),
                    (r) => r.maybeMap(
                        orElse: () => Container(),
                        ...
                        loaded: (e) {
                          ...
                          final singlePasien = state.listPasienModel
                              .where((element) => element.mrn == norm);
